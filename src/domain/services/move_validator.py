...
from ...shared.types.type_definitions import MoveRequest, InvalidMoveException
...
class MoveValidatorService:
    ...
    def validate_move_request(self, game: Game, move_request: MoveRequest) -> bool:
        ...
        if self._is_pawn_promotion_move(board, move_request):
            ...
            if move_request.promotion not in [chess.QUEEN, chess.ROOK, chess.BISHOP, chess.KNIGHT]:
    ...
    def is_square_attackable(self, board: Board, square: int, by_player: Player) -> bool:
        """
    ...
    def is_move_castling(self, move: chess.Move) -> bool:
        ...
        return abs(move.to_square - move.from_square) == 2 and \
    ...
    def _move_belongs_to_player(self, board: Board, move: chess.Move, player: Player) -> bool:
