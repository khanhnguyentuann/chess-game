"""
Redo Move Use Case
Handles the business logic for redoing moves in chess game.
"""
...
class RedoMoveUseCase:
    """
    Use case for redoing moves in the chess game.
    Handles redo operations with event publishing and notifications.
    """
        ...
        try:
            ...
            if result and result.success:
                ...
                redo_event = GameEvent(
                    ...
                    data={
                        "redone_move": result.data.get("redone_move"),
                        "game_id": game.game_id,
                        "move_number": len(game.move_history.moves),
                    },
                )

                await self.event_dispatcher.publish(redo_event)

                # Save updated game state
                await self._save_game_state(game)

                # Notify about successful redo
                await self.notification_service.notify_success(
                    f"Move redone: {result.message}",
                    {
                        "game_id": game.game_id,
    ...
    async def _save_game_state(self, game: Game) -> None:
        ...
        try:
            ...
            self.logger.info(f"Game state saved after redo for game {game.game_id}")
        except Exception as e:
            self.logger.error(f"Error saving game state after redo: {e}")
            # Don't raise here - redo was successful, just couldn't save 