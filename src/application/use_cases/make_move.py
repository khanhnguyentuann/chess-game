...
from ...domain.events.game_events import GameEvent, EventType, MoveEvent
...
from ...shared.exceptions.game_exceptions import InvalidMoveException, GameEndedException
...
class MakeMoveUseCase:
    ...
    def __init__(self,
                 ...
                 move_analyzer=None):  # Add analyzer for move safety analysis
    ...
    async def execute(self, 
                     ...
                     save_game: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if game.is_ended:
                await self.notification_service.notify_game_over(game, game.get_result())
    ...
    async def get_legal_moves(self, game: Game, square: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            if square is not None:
                moves = self.move_validator.get_legal_moves_for_square(game.board, square)
            else:
            ...
            for move in moves:
                move_dict = {
                    ...
                    "is_en_passant": self.move_validator.is_move_en_passant(game.board, move),
                    "notation": self.move_validator.get_move_notation(game.board, move)
                }
                
                # Add safety analysis
                if self.move_analyzer:
                    safety_analysis = self.move_analyzer.analyze_move_safety(game.board, move)
