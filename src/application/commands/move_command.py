...
from .base_command import ICommand, CommandResult
...
class MakeMoveCommand(ICommand):
    ...
    async def execute(self) -> CommandResult:
        ...
        try:
            ...
            # Check for capture
            self.captured_piece = self.game.board.get_piece_at(self.move_request.to_square)
            ...
            if success:
                ...
                return CommandResult(
                    ...
                    data={
                        ...
                        "captured_piece": str(self.captured_piece) if self.captured_piece else None,
                ...
                return CommandResult(
                    success=False,
                    message="Failed to execute move on board"
                )
    ...
    async def undo(self) -> CommandResult:
        ...
        try:
            if not self.previous_board_state or not self.move_executed:
                return CommandResult(
                    success=False,
                    message="Cannot undo: no previous state stored"
                )
            ...
            return CommandResult(
                success=False,
                message=f"Move undo failed: {str(e)}",
                error=e
            )
    
    def can_undo(self) -> bool:
        """Check if move can be undone."""
        return (self.previous_board_state is not None and 
                self.move_executed is not None and
    ...
    def get_description(self) -> str:
        ...
        return f"Move: {chess.square_name(self.move_request.from_square)}" + \
    ...
    def _add_move_to_history_with_notation(self, move: chess.Move, notation: str) -> None:
    ...
    async def _check_game_end_conditions(self) -> None:
        ...
        if self.game.board.is_checkmate():
            winner = Player.BLACK if self.game.current_player == Player.WHITE else Player.WHITE
    ...
    async def execute(self) -> CommandResult:
        ...
        try:
            ...
            if not self.game.board.is_move_legal(self.king_move):
                return CommandResult(
                    success=False,
                    message=f"Castling {self.castling_side} is not legal"
                )
            ...
            if success:
                ...
                return CommandResult(
                    success=False,
                    message="Failed to execute castling"
                )
                
        except Exception as e:
            return CommandResult(
                success=False,
                message=f"Castling failed: {str(e)}",
                error=e
            )
    ...
    async def undo(self) -> CommandResult:
        ...
        try:
            if not self.previous_board_state:
                return CommandResult(
                    success=False,
                    message="Cannot undo castling: no previous state"
                )
            ...
            return CommandResult(
                success=False,
                message=f"Castling undo failed: {str(e)}",
                error=e
            )
...
class ResignCommand(ICommand):
    ...
    async def execute(self) -> CommandResult:
        ...
        try:
            ...
            winner = Player.BLACK if self.resigning_player == Player.WHITE else Player.WHITE
            ...
            return CommandResult(
                success=False,
                message=f"Resignation failed: {str(e)}",
                error=e
            )
    ...
    async def undo(self) -> CommandResult:
        ...
        try:
            if not self.previous_game_state:
                return CommandResult(
                    success=False,
                    message="Cannot undo resignation: no previous state"
                )
            ...
            return CommandResult(
                success=False,
                message=f"Resignation undo failed: {str(e)}",
                error=e
            )
    
    def can_undo(self) -> bool:
        """Check if resignation can be undone."""
        return (self.previous_game_state is not None and 
                self.status.value == "completed")
