...
class CommandExecutor:
    ...
    async def undo(self) -> Optional[CommandResult]:
        ...
        if not last_command.can_undo():
            return CommandResult(
                success=False,
                message="Last command cannot be undone"
            )
    ...
    def can_undo(self) -> bool:
        ...
        return bool(self._command_history and 
                   self._command_history[-1].can_undo())
...
class CompositeCommand(ICommand):
    ...
    def __init__(self, commands: List[ICommand], description: str = "Composite Command"):
        super().__init__()
    ...
    async def _undo_executed_commands(self) -> CommandResult:
        ...
        if failed_undos:
            return CommandResult(
                success=False,
                message=f"Some commands failed to undo: {failed_undos}"
            )
        
        return CommandResult(
            success=True,
            message="Composite command undone successfully"
        )
