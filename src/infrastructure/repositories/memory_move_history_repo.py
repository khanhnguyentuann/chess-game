...
class MemoryMoveHistoryRepository(IMoveHistoryRepository):
    ...
    async def save_move_history(self, game_id: str, history: MoveHistory) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to save move history for game {game_id}: {str(e)}"
            )
        ...
        try:
            for history_data in self._move_histories.values():
                if history_data["game_id"] == game_id:
                    ...
                    move_history.current_position = history_data.get(
                        "current_position", 0
                    )
            ...
            self.logger.error(
                f"Failed to load move history for game {game_id}: {str(e)}"
            )
